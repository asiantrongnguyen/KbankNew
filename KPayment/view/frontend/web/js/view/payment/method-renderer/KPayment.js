define(
    ...
    function (
        ...
        if(customer.isLoggedIn()){
            $.ajax({
                type: "POST",
                url: url.build('kpayment/token/index'),
        ...
        return Component.extend({
            defaults: {
                template: 'Marvelic_KPayment/payment/kpayment'
            },
            ...
            removeStoredCard : function(){
                ...
                $.ajax({
                    type: "POST",
                    url: url.build('kpayment/token/remove'),
                    data: {token: tokenId},
                    async : false,
                    success: function (response) {
                        if(response === "0"){
            ...
            afterPlaceOrder: function () {
                window.location.replace(url.build('kpayment/payment/request'));
            }
        });
    }
);